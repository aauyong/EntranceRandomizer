...
public class MapTracker extends JLayeredPane {
     ...
     * @param b
     * @return
     *         {@code true} : If both connections are successfully made. <p>
     *         {@code false} : If either connection fails. <p>
     * 
     * @see #connect(EntranceIcon, EntranceIcon)
     */
    public boolean createConnection(EntranceIcon a, EntranceIcon b) {
        if ( connectByGroups 
                && (a.hasGrouping() && b.hasGrouping()) ) {
            EntranceGroup egA = this.entranceGroups.get(a.getGroupName());
            EntranceGroup egB = this.entranceGroups.get(b.getGroupName());

            if (egA.areConnectable(egB))
    ...
    private EntranceEvent leftClickBehavior(EntranceIcon e) {
        ...
        var evnt = new EntranceEvent( EntranceEvent.CONNECTION, 
                this.activeEntr, e);
    ...
    private boolean connectByOneWay = false;
}
